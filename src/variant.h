...
struct Variant
{
    ...
    PieceSet promotionPieceTypes[COLOR_NB] = {
    ...
    // only has to match if rook is not in corner in non-960 variants
    File castlingRookKingsideFile = FILE_MAX;
    // only has to match if rook is not in corner in non-960 variants
    File castlingRookQueensideFile = FILE_A;
    ...
    // multiply stalemate value by sign(count(~stm) - count(stm))
    bool stalematePieceCount = false;
    ...
    {
        // Avoid ambiguous definition by removing existing piece with same
        // letter
        size_t idx;
